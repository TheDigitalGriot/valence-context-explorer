...
Email templates for Valence, built with [React Email](https://react.email).
...
1. Generate PNG assets (email clients don't support SVG):
   ...
   - Footer logo (`logo-full.png`): 512x83px (pixelated VALENCE text)
   - Social icons: 48x48px (displayed at 24x24 for retina)
2. Place in `apps/marketing/public/assets/emails/`
3. Images load from `https://valence.sh/assets/emails/` in production
...
```bash
NEXT_PUBLIC_MARKETING_URL=https://valence.sh
```
...
import { WelcomeEmail } from "@valence/email/emails/welcome";
...
await resend.emails.send({
  from: "noreply@valence.sh",
  to: user.email,
  subject: "Welcome to Valence!",
  html,
});
```
...
**Example:**
```tsx
<Button href="https://app.valence.sh">Get Started</Button>
<Button href="https://valence.sh" variant="secondary">
  Learn More
</Button>
```
