...
import type { ChatHistoryEntry, ContentBlock } from "../types";
...
export class SessionContentFilter {
	...
	static async hasNonNoiseMessages(
		...
		const fileStream = fsProvider.createReadStream(filePath, {
			encoding: "utf8",
		});
		...
		try {
			...
			logger.error(
				`Error checking displayable messages in ${filePath}:`,
				error,
			);
		}
			...
			for (const block of content) {
				if (block.type === "text") {
					...
					for (const tag of HARD_NOISE_TAGS) {
						const closeTag = tag.replace("<", "</");
						if (
							textBlock.text.startsWith(tag) &&
							textBlock.text.trim().endsWith(closeTag)
						) {
							return false;
						}
					}
