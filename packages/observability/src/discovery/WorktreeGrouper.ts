...
import { extractBaseDir } from "../internal-utils/pathDecoder";
...
export class WorktreeGrouper {
	...
	async groupByRepository(projects: Project[]): Promise<RepositoryGroup[]> {
		...
		const shouldFilterNoise =
			process.env.CLAUDE_DEVTOOLS_STRICT_SESSION_FILTER === "1";
		await Promise.all(
			projects.map(async (project) => {
				...
				for (const sessionId of project.sessions) {
					...
					if (
						await SessionContentFilter.hasNonNoiseMessages(
							sessionPath,
							this.fsProvider,
						)
					) {
		...
		for (const [groupId, group] of repoGroups) {
			const worktrees: Worktree[] = await Promise.all(
				group.projects.map(async (project) => {
					...
					const isMainWorktree = !(await gitIdentityResolver.isWorktree(
						project.path,
					));
					// Use filtered sessions instead of raw sessions
					const filteredSessions =
						projectFilteredSessions.get(project.id) ?? [];
			...
			const nonEmptyWorktrees = worktrees.filter(
				(wt) => wt.sessions.length > 0,
			);
			...
			const totalSessions = nonEmptyWorktrees.reduce(
				(sum, wt) => sum + wt.sessions.length,
				0,
			);
			...
			repositoryGroups.push({
				...
				mostRecentSession:
					mostRecentSession > 0 ? mostRecentSession : undefined,
				totalSessions,
			});
		}

		// 5. Sort repository groups by most recent activity
		repositoryGroups.sort(
