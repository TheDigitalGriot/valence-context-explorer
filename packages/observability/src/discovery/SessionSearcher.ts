...
import { parseJsonlFile } from "../internal-utils/jsonl";
import {
	extractBaseDir,
	extractSessionId,
} from "../internal-utils/pathDecoder";
import type { SearchResult, SearchSessionsResult } from "../types";
...
export class SessionSearcher {
		...
		try {
			...
			for (const stageBoundary of stageBoundaries) {
				for (
					...
					if (
						fastMode &&
						Date.now() - startedAt >= SSH_FAST_SEARCH_TIME_BUDGET_MS
					) {
						isPartial = true;
						shouldStop = true;
						break;
	...
	async searchSessionFile(
		...
		const hasAnyMatch = entries.some((entry) =>
	...
	private collectMatchesForEntry(
		...
		let pos = lowerText.indexOf(query, 0);
		let matchIndex = 0;
		while (pos !== -1) {
			...
			matchIndex++;
			pos = lowerText.indexOf(query, pos + query.length);
		}
	}
