		...
		if (
			...
			if (propJsonSchema.default === null) {
				...
				const isNullable =
					...
					(propJsonSchema as JsonSchemaObject & { nullable?: boolean })
						.nullable === true;
				...
				properties[key] = propZodSchema
					.optional()
					.default(propJsonSchema.default);
			}
		} else {
...
export function parseObject(
	...
	const isAdditionalPropertiesTrue =
	...
	if (normalizedSchema.patternProperties) {
		...
		if (propertiesSchema) {
			if (additionalProperties) {
				zodSchema = propertiesSchema.catchall(
					z.union([...patternPropertyValues, additionalProperties] as [
						z.ZodTypeAny,
						z.ZodTypeAny,
					]),
				);
		...
		const objectPropertyKeys = new Set(
		...
		zodSchema = zodSchema.superRefine((value: Record<string, unknown>, ctx) => {
			for (const key in value) {
				...
				for (const patternPropertyKey in normalizedSchema.patternProperties) {
					...
					if (key.match(regex)) {
						wasMatched = true;
						const result = parsedPatternProperties[
							patternPropertyKey
						].safeParse(value[key]);
						if (!result.success) {
							ctx.addIssue({
	...
	if (propertiesSchema) {
		if (hasPatternProperties) {
			output = zodSchema ?? propertiesSchema;
		...
		if (hasPatternProperties) {
			output = zodSchema ?? z.object({}).passthrough();
		} else if (additionalProperties) {
			if (additionalProperties instanceof z.ZodNever) {
