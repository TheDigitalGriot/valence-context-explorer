...
describe("getPullRequestRepoArgs", () => {
	test("returns upstream repo args for forks", () => {
		expect(
			getPullRequestRepoArgs({
				isFork: true,
				upstreamUrl: "git@github.com:valence-sh/valence.git",
			}),
		).toEqual(["--repo", "valence-sh/valence"]);
	});
	...
	test("returns no repo args for non-forks", () => {
		expect(
			getPullRequestRepoArgs({
				isFork: false,
				upstreamUrl: "https://github.com/valence-sh/valence",
			}),
		).toEqual([]);
	});
...
describe("shouldRefreshCachedRepoContext", () => {
	test("returns false when no cached repo context exists", () => {
		expect(
			shouldRefreshCachedRepoContext({
				originUrl: "https://github.com/valence-sh/valence",
	...
	test("returns false when the cached repo still matches origin", () => {
		expect(
			shouldRefreshCachedRepoContext({
				originUrl: "https://github.com/valence-sh/valence",
				cachedRepoContext: {
					repoUrl: "https://github.com/valence-sh/valence",
					upstreamUrl: "https://github.com/valence-sh/valence",
					isFork: false,
				},
			}),
	...
	test("treats SSH and HTTPS forms of the same repo as equal", () => {
		expect(
			shouldRefreshCachedRepoContext({
				originUrl: "git@github.com:Valence-Sh/valence.git",
				cachedRepoContext: {
					repoUrl: "https://github.com/valence-sh/valence",
					upstreamUrl: "https://github.com/valence-sh/valence",
					isFork: false,
	...
	test("returns true when origin no longer matches the cached repo", () => {
		expect(
			shouldRefreshCachedRepoContext({
				originUrl: "https://github.com/Kitenite/valence",
				cachedRepoContext: {
					repoUrl: "https://github.com/valence-sh/valence",
					upstreamUrl: "https://github.com/valence-sh/valence",
					isFork: false,
						...
						nodes: [
							{
								...
								url: "https://github.com/valence-sh/valence/pull/2681#discussion_r42",
			...
			parseConversationCommentsResponse([
				{
					...
					html_url:
						"https://github.com/valence-sh/valence/pull/2681#issuecomment-7",
				},
			]),
		).toEqual([
			{
				...
				url: "https://github.com/valence-sh/valence/pull/2681#issuecomment-7",
				kind: "conversation",
				isResolved: false,
			},
		]);
