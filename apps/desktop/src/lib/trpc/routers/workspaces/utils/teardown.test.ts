...
import { PROJECTS_DIR_NAME, VALENCE_DIR_NAME } from "shared/constants";

const TEST_DIR = join(tmpdir(), `valence-test-teardown-${process.pid}`);
const TEST_VALENCE_HOME = join(TEST_DIR, "valence-home");
...
const ORIGINAL_VALENCE_HOME_DIR = process.env.VALENCE_HOME_DIR;
...
describe("runTeardown", () => {
	beforeEach(() => {
		process.env.VALENCE_HOME_DIR = TEST_VALENCE_HOME;
		// Create test directories
		mkdirSync(join(MAIN_REPO, ".valence"), { recursive: true });
	...
	afterEach(() => {
		...
		if (ORIGINAL_VALENCE_HOME_DIR === undefined) {
			delete process.env.VALENCE_HOME_DIR;
		} else {
			process.env.VALENCE_HOME_DIR = ORIGINAL_VALENCE_HOME_DIR;
		}
	...
	test("returns success when config has no teardown commands", async () => {
		writeFileSync(
			join(MAIN_REPO, ".valence", "config.json"),
	...
	test("uses worktreePath config when present", async () => {
		...
		mkdirSync(join(WORKTREE, ".valence"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".valence", "config.json"),
	...
	test("sets environment variables for teardown scripts", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				teardown: [
					`echo "$VALENCE_WORKSPACE_NAME|$VALENCE_ROOT_PATH" > "${envFile}"`,
				],
			}),
		);
	...
	test("uses managed wrapper path for teardown managed binaries even with PATH rewrites", async () => {
		...
		const wrapperBinDir = join(TEST_VALENCE_HOME, "bin");
