...
const TEST_DIR = join(tmpdir(), `valence-test-setup-${process.pid}`);
...
describe("loadSetupConfig", () => {
	...
	test("prefers worktree config over main repo config", () => {
		const mainConfig = { setup: ["./.valence/setup.sh"] };
...
describe("config.local.json", () => {
	...
	test("local config with before/after merges with base setup", () => {
		...
		writeFileSync(
			join(MAIN_REPO, ".valence", "config.local.json"),
			JSON.stringify({
	...
	test("worktree local config takes priority over main repo local config", () => {
		...
		mkdirSync(join(WORKTREE, ".valence"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".valence", "config.local.json"),
