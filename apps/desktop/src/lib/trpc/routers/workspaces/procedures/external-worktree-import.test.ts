 ...
 * 2. The createdByValence flag is correctly set
 * 3. External worktrees are not deleted from disk when workspace is removed
 */

const TEST_DIR = join(
	realpathSync(tmpdir()),
	`valence-test-external-wt-${process.pid}`,
);
...
describe("External worktree detection and import", () => {
	...
	test("external worktree can be created and detected", () => {
		// Create external worktree manually (simulates user creating it outside Valence)
	...
	test("external worktree data survives simulated deletion", () => {
		...
		// the createdByValence flag and skip disk deletion for external worktrees
