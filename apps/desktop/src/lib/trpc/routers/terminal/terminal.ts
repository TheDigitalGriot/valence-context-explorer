import { workspaces, worktrees } from "@valence/local-db";
...
const DEBUG_TERMINAL = process.env.VALENCE_TERMINAL_DEBUG === "1";
 ...
 * - PATH: Prepends ~/.valence/bin so wrapper scripts intercept agent commands
 * - VALENCE_PANE_ID: The pane ID (used by notification hooks, session key)
 * - VALENCE_TAB_ID: The tab ID (parent of pane, used by notification hooks)
 * - VALENCE_WORKSPACE_ID: The workspace ID (used by notification hooks)
 * - VALENCE_WORKSPACE_NAME: The workspace name (used by setup/teardown scripts)
 * - VALENCE_WORKSPACE_PATH: The worktree path (used by setup/teardown scripts)
 * - VALENCE_ROOT_PATH: The main repo path (used by setup/teardown scripts)
 * - VALENCE_PORT: The hooks server port for agent completion notifications
 */
