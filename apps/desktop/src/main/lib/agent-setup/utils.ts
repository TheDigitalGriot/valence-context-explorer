...
function findBinaryPathsUnix(name: string): string[] {
	...
	const delimiter = "__VALENCE_WHICH_DELIMITER__";
 ...
 * Filters out all valence bin directories (prod, dev, and workspace-specific)
...
export function findRealBinary(name: string): string | null {
	try {
		...
		// Filter out wrapper scripts from all valence directories:
		// - ~/.valence/bin
		// - ~/.valence-*/bin (workspace-specific instances)
		const valenceBinDir = path.join(homedir, ".valence", "bin");
		const valencePrefix = path.join(homedir, ".valence-");
		const paths = allPaths.filter(
			(p) =>
				p &&
				!p.startsWith(valenceBinDir) &&
				!(p.startsWith(valencePrefix) && p.includes("/bin/")) &&
