{{MARKER}}
/**
 * Valence Notification Plugin for OpenCode
 *
 ...
 * - Debug logging: Set VALENCE_DEBUG=1 to enable verbose logging
 *
 * SUBAGENT FILTERING:
 ...
 */
export const ValenceNotifyPlugin = async ({ $, client }) => {
  if (globalThis.__valenceOpencodeNotifyPluginV8) return {};
  globalThis.__valenceOpencodeNotifyPluginV8 = true;

  // Only run inside a Valence terminal session
  if (!process?.env?.VALENCE_TAB_ID) return {};

  const notifyPath = "{{NOTIFY_PATH}}";
  const debug = process?.env?.VALENCE_DEBUG === '1';
  ...
  const log = (...args) => {
    if (debug) console.log('[valence-plugin]', ...args);
  };

  /**
   * Sends a notification to Valence's notification server.
