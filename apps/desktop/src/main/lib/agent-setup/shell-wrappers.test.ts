...
const TEST_ROOT = path.join(
	tmpdir(),
	`valence-shell-wrappers-${process.pid}-${Date.now()}`,
);
...
describe("shell-wrappers", () => {
	...
	it("creates zsh wrappers with interactive .zlogin sourcing and idempotent PATH prepend", () => {
		...
		expect(zshenv).toContain('source "$_valence_home/.zshenv"');
		expect(zshenv).toContain(
			`export ZDOTDIR=${quoteShellLiteral(TEST_ZSH_DIR)}`,
		);
		expect(zprofile).toContain('export ZDOTDIR="$_valence_home"');
		expect(zprofile).toContain('source "$_valence_home/.zprofile"');
		expect(zprofile).toContain(
			`export ZDOTDIR=${quoteShellLiteral(TEST_ZSH_DIR)}`,
		);
		expect(zprofile.indexOf('export ZDOTDIR="$_valence_home"')).toBeLessThan(
			zprofile.indexOf('source "$_valence_home/.zprofile"'),
		);

		expect(zshrc).toContain("_valence_prepend_bin()");
		...
		expect(zshrc).toContain('export ZDOTDIR="$_valence_home"');
		expect(zshrc).toContain('source "$_valence_home/.zshrc"');
		expect(zshrc.indexOf('export ZDOTDIR="$_valence_home"')).toBeLessThan(
			zshrc.indexOf('source "$_valence_home/.zshrc"'),
		);
		...
		expect(zshrc).toContain("_valence_ensure_path()");

		expect(zlogin).toContain("if [[ -o interactive ]]; then");
		expect(zlogin).toContain('export ZDOTDIR="$_valence_home"');
		expect(zlogin).toContain('source "$_valence_home/.zlogin"');
		expect(zlogin.indexOf('export ZDOTDIR="$_valence_home"')).toBeLessThan(
			zlogin.indexOf('source "$_valence_home/.zlogin"'),
		);
		expect(zlogin).toContain("_valence_prepend_bin()");
	...
	it("creates bash wrapper without persistent command shims and with idempotent PATH prepend", () => {
		...
		expect(zlogin).toContain("_valence_prepend_bin()");
		expect(zlogin).not.toContain(`claude() { "${TEST_BIN_DIR}/claude" "$@"; }`);
		expect(rcfile).toContain("_valence_prepend_bin()");
	...
	it("reproduces pre-fix .zlogin behavior where system node wins", () => {
		...
		const integrationBinDir = path.join(integrationRoot, "valence-bin");
...
`,
	...
	it("uses login zsh command args when wrappers exist", () => {
		...
		expect(args[1]).toContain(
			`_valence_wrapper=${quoteShellLiteral(path.join(TEST_BIN_DIR, "claude"))}`,
		);
...
`,
	...
	describe("VALENCE_* env var protection from user RC overrides", () => {
		it("bash wrapper restores VALENCE_WORKSPACE_NAME after user .bashrc overrides it", () => {
			const integrationRoot = path.join(TEST_ROOT, "bash-env-protect");
			const homeDir = path.join(integrationRoot, "home");
			mkdirSync(homeDir, { recursive: true });

			// User .bashrc overrides VALENCE_WORKSPACE_NAME with corrupted value
			writeFileSync(
				path.join(homeDir, ".bashrc"),
				`export VALENCE_WORKSPACE_NAME="user@host:~/path/to/worktree"\n`,
			);
			...
			const output = execFileSync("bash", args, {
				...
				env: {
					HOME: homeDir,
					PATH: "/usr/bin:/bin",
					VALENCE_WORKSPACE_NAME: "my-clean-workspace",
				},
			}).trim();
		...
		it("bash wrapper restores VALENCE_WORKSPACE_NAME after user .bash_profile overrides it", () => {
			const integrationRoot = path.join(TEST_ROOT, "bash-profile-env-protect");
			const homeDir = path.join(integrationRoot, "home");
			mkdirSync(homeDir, { recursive: true });

			// User .bash_profile overrides VALENCE_WORKSPACE_NAME
			writeFileSync(
				path.join(homeDir, ".bash_profile"),
				`export VALENCE_WORKSPACE_NAME="$(whoami)@$(hostname):$(pwd)"\n`,
			);
			...
			const output = execFileSync("bash", args, {
				...
				env: {
					HOME: homeDir,
					PATH: "/usr/bin:/bin",
					VALENCE_WORKSPACE_NAME: "correct-name",
				},
			}).trim();
		...
		it("bash wrapper restores multiple VALENCE_* vars after user RC overrides them", () => {
			...
			writeFileSync(
				path.join(homeDir, ".bashrc"),
				`export VALENCE_WORKSPACE_NAME="corrupted"
export VALENCE_WORKSPACE_PATH="/wrong/path"
`,
			);
			...
			const args = [
				...
				path.join(TEST_BASH_DIR, "rcfile"),
				"-ic",
				'echo "$VALENCE_WORKSPACE_NAME|$VALENCE_WORKSPACE_PATH"',
			];
			const output = execFileSync("bash", args, {
				encoding: "utf-8",
				env: {
					...
					PATH: "/usr/bin:/bin",
					VALENCE_WORKSPACE_NAME: "correct-name",
					VALENCE_WORKSPACE_PATH: "/correct/path",
				},
			}).trim();
		...
		it("zsh wrapper restores VALENCE_WORKSPACE_NAME after user .zshrc overrides it", () => {
			if (!isZshAvailable()) return;
			...
			// User .zshrc overrides VALENCE_WORKSPACE_NAME with corrupted value
			writeFileSync(
				path.join(homeDir, ".zshrc"),
				`export VALENCE_WORKSPACE_NAME="user@host:~/path/to/worktree"\n`,
			);
			...
			const output = execFileSync(
				"zsh",
				["-lic", 'echo "$VALENCE_WORKSPACE_NAME"'],
				{
					encoding: "utf-8",
					env: {
						HOME: homeDir,
		...
		it("zsh wrapper restores VALENCE_WORKSPACE_NAME after user .zlogin overrides it", () => {
			if (!isZshAvailable()) return;
			...
			writeFileSync(
				path.join(homeDir, ".zlogin"),
				`export VALENCE_WORKSPACE_NAME="overridden-by-zlogin"\n`,
			);
	...
	describe("fish shell", () => {
		...
		it("uses --init-command to prepend BIN_DIR to PATH for fish", () => {
			...
			expect(args).toEqual([
				"-l",
				"--init-command",
				`set -l _valence_bin "${TEST_BIN_DIR}"; contains -- "$_valence_bin" $PATH; or set -gx PATH "$_valence_bin" $PATH; function _valence_shell_ready --on-event fish_prompt; printf '\\033]777;valence-shell-ready\\007'; functions -e _valence_shell_ready; end`,
			]);
		});
		...
		it("escapes fish init-command BIN_DIR safely", () => {
			...
			expect(args).toEqual([
				"-l",
				"--init-command",
				`set -l _valence_bin "/tmp/with space/quote\\"buck\\$slash\\\\bin"; contains -- "$_valence_bin" $PATH; or set -gx PATH "$_valence_bin" $PATH; function _valence_shell_ready --on-event fish_prompt; printf '\\033]777;valence-shell-ready\\007'; functions -e _valence_shell_ready; end`,
			]);
		});
	});
});
