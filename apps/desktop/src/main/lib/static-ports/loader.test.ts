...
const TEST_DIR = join(tmpdir(), `valence-test-loader-${process.pid}`);
const WORKTREE_PATH = join(TEST_DIR, "worktree");
const VALENCE_DIR = join(WORKTREE_PATH, ".valence");
const PORTS_FILE = join(VALENCE_DIR, "ports.json");

describe("loadStaticPorts", () => {
	beforeEach(() => {
		mkdirSync(VALENCE_DIR, { recursive: true });
	});
