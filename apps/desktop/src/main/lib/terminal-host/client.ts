...
const DEBUG_CLIENT = process.env.VALENCE_TERMINAL_DEBUG === "1";
...
const SPAWN_LOCK_PATH = join(VALENCE_HOME_DIR, "terminal-host.spawn.lock");
const SCRIPT_MTIME_PATH = join(VALENCE_HOME_DIR, "terminal-host.mtime");
...
export class TerminalHostClient extends EventEmitter {
	...
	private acquireSpawnLock(): boolean {
		try {
			// Ensure valence home directory exists before any file operations
	...
	private async spawnDaemon(): Promise<void> {
		...
		try {
			...
			const logPath = join(VALENCE_HOME_DIR, "daemon.log");
			let logFd: number;
			try {
