...
import {
	VALENCE_HOME_DIR,
	VALENCE_HOME_DIR_MODE,
	VALENCE_SENSITIVE_FILE_MODE,
} from "./app-environment";

const RINGTONES_ASSETS_DIR = join(VALENCE_HOME_DIR, "assets", "ringtones");
...
function writeCustomRingtoneMetadata(name: string): void {
	...
	try {
		chmodSync(CUSTOM_RINGTONE_METADATA_PATH, VALENCE_SENSITIVE_FILE_MODE);
	} catch {
		// Best effort only.
	}
}
...
export function ensureCustomRingtonesDir(): void {
	...
	try {
		chmodSync(RINGTONES_ASSETS_DIR, VALENCE_HOME_DIR_MODE);
	} catch {
		// Best effort only.
	}
}
...
export async function importCustomRingtoneFromPath(
	...
	if (areSamePath(sourcePath, destinationPath) && existsSync(destinationPath)) {
		try {
			chmodSync(destinationPath, VALENCE_SENSITIVE_FILE_MODE);
		} catch {
			// Best effort only.
		}
