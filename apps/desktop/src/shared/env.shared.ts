...
const envSchema = z.object({
	...
	VALENCE_WORKSPACE_NAME: z.string().default("valence"),
});
...
export const env = envSchema.parse({
	...
	VALENCE_WORKSPACE_NAME: process.env.VALENCE_WORKSPACE_NAME,
});

export function getWorkspaceName(): string | undefined {
	const name = env.VALENCE_WORKSPACE_NAME;
	if (name === "valence") return undefined;
	return name
		.toLowerCase()
		.replace(/[^a-z0-9-]/g, "-")
		.slice(0, 32);
