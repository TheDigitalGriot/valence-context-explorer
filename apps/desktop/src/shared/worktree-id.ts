...
export function normalizeWorkspaceName(name?: string): string | undefined {
	if (!name || name === "valence") return undefined;
	return name
		.toLowerCase()
		.replace(/[^a-z0-9-]/g, "-")
		.slice(0, 32);
 ...
 * ~/.valence/worktrees/<project>/...
 *
 * Examples:
 * - ["valence", "my-branch", "apps", "desktop"] -> "my-branch"
 * - ["valence", "owner", "workspace", "apps", "desktop"] -> "owner-workspace"
 * - ["valence", "owner", "feature", "x", "apps", "desktop"] -> "owner-feature-x"
 */
...
export function getWorkspaceName(): string | undefined {
	return normalizeWorkspaceName(process.env.VALENCE_WORKSPACE_NAME);
}
