...
export function SlashCommandPreview({
	...
	const previewSlashCommand =
		workspaceTrpc.chat.previewSlashCommand.useMutation();
	const [slashPreview, setSlashPreview] = useState<{
		commandName?: string;
		prompt?: string;
	} | null>(null);
		...
		void previewSlashCommand
			...
			.then((result) => {
				if (!cancelled) {
					setSlashPreview({
						commandName:
							"commandName" in result && typeof result.commandName === "string"
								? result.commandName
								: undefined,
						prompt:
