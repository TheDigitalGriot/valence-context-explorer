...
export function TraceInspector({ nodeDetails, onNodeNavigate }: TraceInspectorProps) {
  if (!nodeDetails) {
    return (
  ...
  const influence = nodeDetails.properties.influence_score as number | undefined;
        ...
        <ScrollArea className="h-[calc(100%-4rem)]">
          {(influence != null || communityId != null) && (
            <div className="mb-3 space-y-1">
              <p className="text-xs font-semibold text-muted-foreground">Graph Analytics</p>
              {influence != null && (
              ...
              {communityId != null && (
                <div className="flex justify-between text-xs">
                  <span>Community</span>
                  <Badge variant="secondary" className="h-4 px-1 text-[10px]">#{communityId}</Badge>
                </div>
              )}
            </div>
          )}
