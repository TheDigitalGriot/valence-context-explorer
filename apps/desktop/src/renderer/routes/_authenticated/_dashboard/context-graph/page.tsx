...
import { TraceInspector, type NodeDetails } from "./components/TraceInspector";
import { GraphSearchPanel } from "./components/GraphSearchPanel";
import { CommunityAnalysis } from "./components/CommunityAnalysis";

export const Route = createFileRoute("/_authenticated/_dashboard/context-graph/")({
...
function ContextGraphPage() {
  ...
  const nodeDetails: NodeDetails | null = selectedNodeId
    ? (() => {
        ...
        const connections = graphData.relationships
          .filter((r) => r.startNode === selectedNodeId || r.endNode === selectedNodeId)
          .map((r) => {
  ...
  const statsArray = stats
    ? Object.entries(stats as Record<string, number>).map(([label, count]) => ({
        label,
        count,
      }))
    : undefined;
  ...
  return (
    <div className="flex h-full flex-col gap-3 p-4">
      <div className="flex items-center justify-between">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as "graph" | "communities")}>
          <TabsList>
      ...
      {tab === "graph" ? (
        ...
        <CommunityAnalysis
          communities={(communitiesQuery.data as { communities?: Array<{ communityId: number; traceCount: number; errorCount: number; errorRate: number; avgPageRank: number; representativeTraces: Array<{ id: string; name: string; status?: string }> }> })?.communities ?? []}
        />
      )}
    </div>
  );
