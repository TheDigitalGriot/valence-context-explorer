...
export function useGraphData(params: {
	...
	const graphData = useMemo<GraphData>(() => {
		...
		// search returns a flat array of nodes with scores (no edges)
		const searchNodes = (searchResult.data ?? []) as GraphNode[];
		const searchRels: GraphRelationship[] = [];
	...
	const expandNode = useCallback(
		async (nodeId: string) => {
			...
			const result = await utils.contextGraph.expandNode.fetch({
				nodeId,
				depth: 1,
			});
