...
- remove `@valence/workspace-fs/host` import
- stop deleting files through direct host calls
- use filesystem adapter path or move cleanup to renderer

Terminal router:

- remove `@valence/workspace-fs/host` imports
- stop writing task prompt files inside `terminal.ts`
- require caller to create `.valence/<file>` via `trpc.filesystem.*` before attach
...
```bash
rg -n "@valence/workspace-fs/host" apps/desktop/src
...
- no desktop router outside the filesystem adapter imports `@valence/workspace-fs/host` for workspace file ops
