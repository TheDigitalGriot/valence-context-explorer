...
1. A “workspace” is a Valence concept representing a git worktree; routes use `/workspace/$workspaceId`.
2. Tabs and panes are conceptually workspace-local. A tab should never render while viewing another workspace route.
3. UI state persistence is local-only (lowdb JSON) at `~/.valence/app-state.json` (see `apps/desktop/src/main/lib/app-environment.ts`), not the production database.
...
- `apps/desktop/src/main/lib/app-state/index.ts` loads and stores the lowdb file at `APP_STATE_PATH`, which is defined in `apps/desktop/src/main/lib/app-environment.ts` as `~/.valence/app-state.json`.
...
2. Update `apps/desktop/src/main/lib/app-state/index.ts` so `ensureValidShape` normalizes `data.tabsState` via `normalizeTabsState` (imported from the shared helper). This ensures old `~/.valence/app-state.json` files are reshaped on startup.
...
Acceptance:
    ...
    bun run typecheck --filter=@valence/desktop
...
Acceptance:
    ...
    bun run typecheck --filter=@valence/desktop
    bun run lint:check-node-imports --filter=@valence/desktop
...
Validation commands (run from repo root):
    ...
    bun run typecheck --filter=@valence/desktop
    bun run lint
    bun test --filter=@valence/desktop
...
2. Move the local lowdb file out of the way (do not delete immediately):
    ...
    mv ~/.valence/app-state.json ~/.valence/app-state.json.bak
