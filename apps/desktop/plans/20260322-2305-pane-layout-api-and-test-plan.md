...
Define a shippable Valence pane-layout package before we continue implementation.
...
- Renderer layer must be design-system-native, using Tailwind + `@valence/ui`
...
- `react`
  ...
  - group/tab chrome built from `@valence/ui`
...
This package is internal to Valence. It does not need an external-consumer-neutral API.
...
New for Valence because cross-root drag/drop is a core requirement.
...
1. Lock the core API in `@valence/pane-layout`
2. Write reducer and tree utility tests first
3. Implement root-aware reducer
4. Implement minimal React renderer with `@valence/ui` primitives
...
- a Valence-specific React renderer
