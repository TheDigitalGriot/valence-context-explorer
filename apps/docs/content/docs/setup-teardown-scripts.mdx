...
Create `.valence/config.json` in your project:

```json
{
  "setup": ["bun install", "cp \"$VALENCE_ROOT_PATH/.env\" .env"],
...
|----------|-------------|
| `VALENCE_ROOT_PATH` | Path to root repository |
| `VALENCE_WORKSPACE_NAME` | Current workspace name |
| `VALENCE_WORKSPACE_PATH` | Path to the workspace worktree |
...
```
~/.valence/projects/<project-id>/config.json
```

Where `<project-id>` is your project's unique ID in Valence (visible in the app's project settings).

### Priority Order

Config is resolved in this order (first found wins, for setup, teardown, and run):
1. `~/.valence/projects/<project-id>/config.json` — user override
2. `<worktree>/.valence/config.json` — worktree-specific
3. `<repo>/.valence/config.json` — project default
...
```json
{
  "setup": ["~/.valence/projects/<project-id>/setup.sh"],
  "teardown": ["~/.valence/projects/<project-id>/teardown.sh"]
}
```
...
Extend or override your team's committed scripts without modifying the repo. Create `.valence/config.local.json` alongside `config.json` — it's gitignored automatically.
...
```json
{
  "setup": {
    ...
    "after": ["./.valence/my-post-setup.sh"]
  },
  "teardown": {
    "after": ["./.valence/my-cleanup.sh"]
  }
}
```
...
```json
{
  "setup": ["./.valence/my-custom-setup.sh"]
}
```
...
- Commit `.valence/` to share with team
- Use shell scripts for complex logic: `"setup": ["./.valence/setup.sh"]`
- Use `config.local.json` to add personal steps without touching the team's config
- Use user overrides (`~/.valence/projects/`) to replace scripts entirely per-project
