...
Valence does not assign per-workspace port ranges. It discovers listening ports from processes running in each workspace and lets you manage them from the UI.
...
Create `.valence/ports.json` in your repository:
...
**Tips:**
- Commit `.valence/ports.json` to share port labels with your team
- **Pro tip:** If you want deterministic per-workspace port ranges, implement it in setup/teardown scripts by reserving a range in a shared file (for example `~/.valence/port-allocations.json`) during setup and releasing it during teardown. See this repo's examples: [`.valence/setup.sh`](https://github.com/valence-sh/valence/blob/main/.valence/setup.sh) and [`.valence/teardown.sh`](https://github.com/valence-sh/valence/blob/main/.valence/teardown.sh).
