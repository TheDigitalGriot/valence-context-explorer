---
title: MCP Server
description: Connect AI agents directly to Valence via MCP
---

## Overview

Valence provides an [MCP (Model Context Protocol)](https://modelcontextprotocol.io) server that lets AI agents interact with your tasks, workspaces, and devices programmatically.
...
```bash title="terminal"
claude mcp add valence --transport http https://api.valence.sh/api/agent/mcp
```
</Tab>
<Tab value="Codex">
```bash title="terminal"
codex mcp add valence --url https://api.valence.sh/api/agent/mcp
```
</Tab>
<Tab value="Gemini CLI">
```bash title="terminal"
gemini mcp add --transport http valence https://api.valence.sh/api/agent/mcp
```
</Tab>
<Tab value="Open Code">
...
[2m│[0m  [36mhttps://api.valence.sh/api/agent/mcp[0m
[2m│[0m
...
```json title=".mcp.json"
{
  "mcpServers": {
    "valence": {
      "type": "http",
      "url": "https://api.valence.sh/api/agent/mcp"
    }
  }
}
```
...
```json title="settings.json"
{
  "mcpServers": {
    "valence": {
      "httpUrl": "https://api.valence.sh/api/agent/mcp"
    }
  }
}
```
...
1. Open the Valence desktop app
...
| Tool | Description |
|------|-------------|
| `start_agent_session` | Start an autonomous AI agent session for a task in an existing workspace. Requires `taskId`. Supports Claude, Codex, Gemini, OpenCode, Pi, Copilot, Cursor Agent, and Valence Chat (defaults to Claude). When `paneId` is provided, adds a new pane to the tab containing that pane instead of initializing a new tab. |
