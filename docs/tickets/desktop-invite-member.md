...
export function OrganizationInvitationEmail({
  ...
  inviteLink = "https://app.valence.sh/accept-invitation/123",
  role = "member",
  ...
  return (
    <StandardLayout preview={`${inviterName} invited you to join ${organizationName}`}>
      ...
      <Text>
        {inviterName} has invited you to join <strong>{organizationName}</strong> on Valence as a{" "}
        <strong>{roleDisplay}</strong>.
      </Text>

      <Text>
        Valence helps teams automate workflows and boost productivity with AI-powered task management.
      </Text>
...
```typescript
import type { SelectInvitation, SelectMember, SelectUser } from "@valence/db/schema/auth";
...
```typescript
import { canInvite, getInvitableRoles, ORGANIZATION_ROLES, type OrganizationRole } from "@valence/shared/auth";
import { Button } from "@valence/ui/button";
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from "@valence/ui/dialog";
...
```typescript
import type { SelectInvitation } from "@valence/db/schema";
import type { OrganizationRole } from "@valence/shared/auth";
import { Button } from "@valence/ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@valence/ui/dropdown-menu";
import { toast } from "@valence/ui/sonner";
import { useState } from "react";
...
import { resend } from "@valence/email/lib/resend";
...
2. Set up Resend API key in `.env`:
   ...
   - Verify sender domain `noreply@valence.sh`
