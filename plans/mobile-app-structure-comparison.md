...
This document compares the organizational patterns between the Cadra mobile app (reference monorepo) and the Valence mobile app to understand best practices for Expo Router + screens/ colocation.
...
### Valence (Current Implementation)
```
apps/mobile/
...
| Aspect | Cadra Pattern | Valence Pattern |
|--------|---------------|------------------|
...
⚠️ **Key Difference:** Cadra keeps root layout in `app/`, Valence moves it to `screens/`
...
⚠️ **Key Difference:** Cadra keeps redirect-only routes in `app/`, Valence moves them to `screens/`
...
**Valence:**
```tsx
// If we had tabs, following Valence pattern:
...
⚠️ **Key Difference:** Cadra keeps `_layout.tsx` in `app/` for navigation config, Valence would move to `screens/`

---

### Pattern 5: Component Colocation

**Both Cadra & Valence use the SAME pattern:**
...
To migrate Valence to follow Cadra's pattern:
...
- **Valence**: `screens/` owns everything, `app/` is just a thin routing proxy
