...
packages/chat-service/
  ...
  tsconfig.json                                    @valence/db, @valence/durable-session
...
- Import `searchFiles`, `AgentManager` from `@valence/chat-service`
- Import Zod schemas from `@valence/chat-service/router`
...
```tsx
import { chatService } from "@valence/chat-service/client";
