...
For v1, do not move presets into `.valence/config.json`. Keep the primary editor in `Settings > Terminal`, and use Project settings only as a contextual shortcut into that same editor.
...
Do not store them in `.valence/config.json` yet.
...
Do not move presets into `.valence/config.json` yet.
